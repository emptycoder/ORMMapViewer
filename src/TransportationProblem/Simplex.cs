...
namespace TransportationProblem
{
	public class Simplex
	{
		...
		int[] basis; // Input: rowIndex

		public Simplex(BasisFinder basisFinder, int[] basis)
		{
			this.matrix = basisFinder.matrix;
			this.simplexFunction = basisFinder.simplexFunction;
			this.constraints = basisFinder.constraints;
			this.basis = basis;
		}
		...
		private Pivot FindPivot()
		{
			...
			{
				if (columnData[rowIndex] <= 0) { continue; }
		...
		private void RecalculateMatrix(Pivot pivot)
		{
			...
			{
				if (columnIndex == pivot.columnIndex) { continue; }
